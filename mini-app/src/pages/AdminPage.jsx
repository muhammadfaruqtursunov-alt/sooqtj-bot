...
import {
  ...
  Trash2, Download, Search, Star, MessageSquare, X,
} from 'lucide-react'
import BroadcastPanel from '../components/BroadcastPanel'
import PriceBlock from '../components/PriceBlock'
import DoughnutChart from '../components/DoughnutChart'
import { inferCategory } from '../categoryInference'
import { computeCost, computeProfit, getSalePrice, getArticle, fmt } from '../costCalculation'
...
const ph  = p => p['Фото (URL)'] || p['Фото 1'] || p['Фото'] || p['photo_url'] || ''
                            ...
                            {(() => {
                              ...
                              return (
                                <div className="flex flex-col items-end leading-none">
                                  ...
                                  {op && (
                                    <>
                                      <p className="text-[10px] font-bold text-yellow-500 mt-0.5">с/с: {fmt(cost)}</p>
          ...
          <div style={paneStyle} className="p-3">
            <div className="pb-4">
              <button onClick={() => { setEditIdx(null); setShowForm(true) }}
                className="w-full gold text-[#111] font-black rounded-[20px] py-3.5 text-sm mb-4
                  shadow-[0_4px_20px_rgba(245,197,24,0.3)] active:scale-[0.97] transition-transform
                  flex items-center justify-center gap-2">
                <Plus size={18} strokeWidth={2.5} /> Добавить товар
              </button>
              ...
              {loading ? (
                ...
                <div className="flex flex-col items-center gap-3 py-12">
                  ...
                  <p className="text-sm font-semibold text-gray-400 dark:text-gray-600">Товаров нет</p>
                </div>
              ) : (
                <div className="space-y-2.5">
                  {products.map((p, i) => (
                    <div key={i} className="bg-white dark:bg-[#2D2F37] rounded-[20px] overflow-hidden flex
                      ...
                      border border-black/[0.04] dark:border-white/[0.05] animate-fadeIn">
                      <button onClick={() => openPhotoUpload(i)} className="relative flex-shrink-0 group">
                        {ph(p)
                          ? <img src={ph(p)} className="w-20 h-20 object-cover"
                              onError={e => e.target.style.display='none'} />
                          : <div className="w-20 h-20 flex flex-col items-center justify-center gap-0.5"
                              ...
                              <span className="text-[8px] font-black text-[#111] uppercase tracking-wider">Загрузить</span>
                            </div>
                        }
                      ...
                      <div className="flex-1 p-3 min-w-0 flex items-start justify-between gap-2">
                        <div className="min-w-0 flex-1">
                          ...
                          <p className="text-[11px] text-gray-400 mt-0.5 truncate">{ct(p)}</p>
                          <p className={`text-[11px] font-bold mt-0.5 ${
                            qty(p) === 0 ? 'text-red-400' : qty(p) < 5 ? 'text-orange-400' : 'text-gray-400 dark:text-gray-500'
                          }`}>В наличии: {qty(p)} шт</p>
                        </div>
                        <PriceBlock product={p} />
                      </div>
                      <div className="flex flex-col gap-1.5 mt-3 mr-3">
                        <button onClick={() => { setEditIdx(i); setShowForm(true) }}
                          className="w-7 h-7 rounded-xl bg-[#F5F5F5] dark:bg-white/5 flex items-center justify-center active:scale-90 transition-transform">
                          <Pencil size={13} color={dark ? '#888' : '#555'} strokeWidth={2} />
                        </button>
